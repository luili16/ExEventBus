...
public class SubscribeEntry10 {
    ...
    @Subscriber(tag = "event9_SubscribeEntry10",model = ThreadModel.MAIN,type = Type.BLOCK_RETURN,remote = true)
    public String testMethod2(Event9 event9) {
        assertNotNull(event9);
        Log.d("main","SubscribeEntry10 : " + event9.getMsg());
        return "return_" + event9.getMsg();
    }

    @Subscriber(tag = "event10_SubscribeEntry10",model = ThreadModel.HANDLER,type = Type.BLOCK_RETURN,remote = true)
    public String testMethod3(Event10 event10) {
        assertNotNull(event10);
        return "return_"+event10.getMsg();
    }

    @Subscriber(tag = "event11_SubscribeEntry10",model = ThreadModel.POST,type = Type.BLOCK_RETURN,remote = true)
